...
class FXTransaction:
    ...
    def get_pandl(self, serial: int, exit_price: float)->float:
        ...
        buy_fee = buymnt*self.__ptc
        sel_fee = selmnt*self.__ptc
    ...
    def trade_exit(self, serial: int, exit_price: float, time: str, losscut=None) -> bool:
        """
        トレードエグジットを行います。
        ...
        tradetype = self.__fxtrn.get_fd(serial,'tradetype')
        pandl = self.get_pandl(serial, exit_price)  # 利益計算
        equity += pandl
