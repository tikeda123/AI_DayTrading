...
class IdleState(State):
    ...
    def handle_request(self, context, event):
        ...
        if event == EVENT_ENTER_PREPARATION:
            context.log_transaction("ボリンジャーバンドに触れた。エントリー準備状態に遷移します。")
    ...
    def event_handle(self, context, index: int):
        ...
        if context.is_first_column_greater_than_second(index, COLUMN_CLOSE, COLUMN_UPPER_BAND2):
            ...
            context.set_bb_direction(BB_DIRECTION_LOWER)
            context.strategy.Idel_event_execute(context)
        else:
...
class EntryPreparationState(State):
    ...
    def event_handle(self, context, index: int):
        ...
        状態に応じたイベント処理メソッド:
        """
        if context.get_entry_counter() >= WAIT_FOR_ENTRY_COUNT:
            context.strategy.EntryPreparation_event_over_counter_execute(context)
        else:
            context.strategy.EntryPreparation_event_under_counter_execute(context)
